...
export default function Scan() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <PageHeader title="Scan mode" description="Point the camera at a product QR / barcode — or type a SKU. Works great on phones (install as an app)." />
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-4">
          <Card className="space-y-4 p-4">
            <div>
              <div className="mb-1.5 text-xs font-medium text-muted">Action</div>
              <Segmented
                value={action}
                onChange={setAction}
                size="sm"
            ...
            {action !== 'lookup' && (
              <div className="flex flex-wrap items-end gap-3">
                <div>
                  <div className="mb-1.5 text-xs font-medium text-muted">Quantity</div>
                  <div className="flex items-center gap-1">
                    <Button variant="secondary" size="icon" onClick={() => setQty((q) => q - 1)} aria-label="Decrease"><Minus className="size-4" /></Button>
                    <Input type="number" value={qty} onChange={(e) => setQty(Number(e.target.value))} className="w-20 text-center" />
                    <Button variant="secondary" size="icon" onClick={() => setQty((q) => q + 1)} aria-label="Increase"><Plus className="size-4" /></Button>
                  </div>
                </div>
                <div className="flex-1">
                  <div className="mb-1.5 text-xs font-medium text-muted">Warehouse</div>
          ...
          <Card>
            ...
            <div className="px-5 pb-5">
              {!latest ? (
                <EmptyState title="Nothing scanned yet" description="Print product labels from the Inventory page to try it." />
              ) : (
                <div>
