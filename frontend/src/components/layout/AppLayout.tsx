  ...
  return (
    <div className="flex items-center gap-2.5 px-2">
      ...
      <div className="leading-tight">
        <div className="text-sm font-semibold">IntelliInventory</div>
        <div className="text-[11px] text-muted">AI-native inventory</div>
      </div>
    </div>
  )
}
    ...
    <div className="flex h-full flex-col gap-4 p-3">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto">
        {NAV.map(({ to, label, icon: Icon, section }, i) => (
          <div key={to}>
            {section !== NAV[i - 1]?.section && <div className="px-2.5 pt-3 pb-1 text-[11px] font-medium tracking-wide text-subtle uppercase">{section}</div>}
            <NavLink
              to={to}
              end={to === '/'}
              onClick={onNavigate}
      ...
      <div className="rounded-xl border border-border bg-surface p-3">
        <div className="flex items-center gap-2.5">
          ...
          <Tooltip content="Sign out">
            <button onClick={logout} className="rounded-md p-1.5 text-muted hover:bg-surface-2 hover:text-fg" aria-label="Sign out">
    ...
    <div className="flex h-full">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-64 border-r border-border bg-bg animate-fade-in">
            <button className="absolute top-4 right-3 p-1 text-muted" onClick={() => setMobileOpen(false)} aria-label="Close menu">
              <X className="size-4" />
            </button>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="no-print sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-bg/80 px-4 backdrop-blur lg:px-6">
          ...
          <div className="text-sm font-medium text-muted lg:hidden">{current?.label}</div>
          <button
          ...
          >
            ...
            <span className="hidden flex-1 truncate text-left sm:block">Search products, pages, or ask Copilot…</span>
